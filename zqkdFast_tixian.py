...
def getScore(userInfo):
    ...
    try:
        ...
        if score>300000:
            ...
            print('青豆少于3000，终止提现！')
            allMessage =allMessage  +'用户ID' + str(userID)+' 青豆(' +  str(score) + '),青豆少于3000，终止提现！\n'
            return


        print('用户ID',userID,' 青豆(',score,'),可以提现',money,'元')
        allMessage =allMessage +'用户ID' + str(userID)+' 青豆(' +  str(score) + '),可以提现' +  str(money) + '元\n'
...
def getMoneyZFB(userInfo):
    ...
    try:
        ...
        data=json.loads(content)
        print(data['message'])
        allMessage =allMessage + str(data['message']+'\n')
    except Exception as e:
