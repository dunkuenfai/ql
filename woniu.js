...
项目地址 
https://lh100-1253806495.cos.ap-beijing.myqcloud.com/index.html?carteamflg=100036&extflg=33884
...
脚本第36行 const CAHSADD = "0.3";  此处为提现金额  大于0.3才提现 可自己修改
    ...
    async addUserMoneyCash() {
        try {
            ...
            if (result.code == 0) {
                ...
                allMessage += `${this.idx}提现：${JSON.stringify(result)}`
            } else {
    ...
    async getUserMoneyAccounts() {
        try {
            ...
            if (result.ret == "succ") {
                ...
                allMessage += `${this.idx}${JSON.stringify(
                    result.msg
                )} 原因：填错了 或者抓到后又重新打开网页了`
            }
        } catch (e) {
    ...
    async userDrawReadTaskBaidu(a) {
        try {
            ...
            if (result.code == 0) {
                ...
                result.msg == "任务领取成功" &&
                    (await $.wait(25080), await this.finishUserReadTaskBaidu());
            }
        } catch (e) {
        }
    }
...
!(async () => {
    ...
    if (typeof $request !== "undefined") {
        ...
        await notify.sendNotify("蜗牛阅读", `${allMessage}`)
