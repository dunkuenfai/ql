...
//  243adece-42bf-460b-b312-5cd9b0b3f397
// const task = require("./anxuewang.js")
...
async function loadByPhone() {
    ...
    if (this.data.phone == "15917100217" && this.data.code == "1234") {
        // wx.showToast({
        //     title: '登录成功',
        //     icon: "none"
        // })
...
async function autoTask() {
    ...
    //获取CK
    if (!access_token) {
        msg = await getCk(phone, code)
        // that.setData({
        // //     msg: that.data.msg + msg + "\n"
        // })
        // if (msg.indexOf("错误") > 0) {
        //     that.setData({
        //         code: ""
    ...
    msg = await postSign();
    // that.setData({
    //     msg: this.data.msg + "签到结果" + msg + "\n"
    // })
    return

    //第一次挑战
    msg = await getTZquestions();
    that.setData({
        msg: that.data.msg + "挑战答题第一次：" + msg + "\n"
    })
    //第二次挑战
    msg = await getTZquestions();
    that.setData({
        msg: that.data.msg + "挑战答题第二次：" + msg + "\n"
    })
    // 每日答题
    msg = await postDailyquestions();
    that.setData({
        msg: that.data.msg + msg + "\n"
    })
